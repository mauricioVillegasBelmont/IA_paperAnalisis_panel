...
Pymeweb.prototype.init_page_sizer = function(){
  $('body').css({
    "--header-h": function(i){
      return $('#main_header').outerHeight() + 'px';
    },
    "--footer-h": function(i){
      return $('#main_footer').outerHeight() + 'px';
    }
  }
);
...
Pymeweb.prototype.addJQ_functions = function(){
  jQuery.fn.extend({
    tabs:function(args){
      $('.'+args.tabcontrol).attr({
        "role":'tab',
        "aria-selected":false,
        "tabindex":"-1",
        "data-tabpanel":(!args.tabpanel)?'tab-container':args.tabpanel,
        "data-tabcontrol":(!args.tabcontrol)?'tab-btn':args.tabcontrol,
        "data-active":(!args.active)?'active':args.active,
      });
      $('.'+args.tabpanel).attr({
        "aria-selected":false,
        "tabindex":"-1",
      ...
      return this.each(function(index){
        var href_id, this_id;
        href_id = this.getAttribute("href");
        if (href_id == undefined || href_id === '') return;
        if ( this.id == undefined || this.id === '') this.id = href_id.replace('#','')+'_tab-btn_'+Math.floor(Math.random()*(999-100+1)+100)
        this_id = this.id;
        $(this).attr({
          "aria-controls":href_id,
        });
        $(href_id).attr({
          'aria-labelledby':this_id,
        });
        if (index==0) {
          $(this).attr({
            "aria-controls":href_id,
        ...
        $(this).on('click',function(e,arg){
          e.preventDefault();
          console.log(this);
          var _active, _controls, _panel, href;
          _active = $(this).data('active');
          _controls = $(this).data('tabcontrol');
          _panel = $(this).data('tabpanel');
          href = this.getAttribute("href");
          $('.'+_panel).attr({
            "tabindex":"-1",
            "hidden":true
          }).removeClass(_active).hide();
          $('.'+_controls).attr({
            "aria-selected":false,
            "tabindex":"-1",
          }).removeClass(_active);
